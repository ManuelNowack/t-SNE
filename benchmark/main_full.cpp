...
int main(int argc, char **argv) {
  if (argc < 5) {
    ...
    cerr << "Will benchmark from 2^MIN up to 2^MAX samples with multiplicative steps of 2." << endl;
    return 1;
  }

  Matrix X = load_matrix(argv[1]);
  Matrix Y = load_matrix(argv[2]);
  int log2_min_samples = atoi(argv[3]);
  int log2_max_samples = atoi(argv[4]);

  if (log2(X.nrows) < log2_max_samples) {
    cerr << "Maximum number of samples (2^" << log2_max_samples
    << ") is higher than the number of samples in the dataset provided (" << X.nrows << ")" << endl;
    return 1;
  }
  ...
  int n_measurements = log2_max_samples - log2_min_samples + 1;
  double performances[n_measurements][n_measurement_series];

  cout << endl;
  for (int k=log2_min_samples; k<=log2_max_samples; k++) {
    cout << "2^" << k << " samples" << endl;

    int n_samples = (int)powl(2, k);
    ...
    for (int i = 0; i < tsne_func_registry.num_funcs; i++) {
      ...
      performances[k-log2_min_samples][i_series] = perf;
      i_series++;
    }
