...
void calc_joint_probabilities(Matrix X, Matrix P, double perplexity,
                              double tol) {
  /*
...
void tsne_baseline(Matrix X, int n_dim, double perplexity, Matrix Y) {
  /*
  ...
  for (int iter = 0; iter < max_iter; iter++) {
    ...
    for (int i = 0; i < n; i++) {
      for (int j = 0; j < n_dim; j++) {
        double value =
            momentum * Y_delta.data[i * n_dim + j] -
            eta * gains.data[i * n_dim + j] * grad_Y.data[i * n_dim + j];
