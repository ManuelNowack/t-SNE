...
*/
void _grad_desc_vec(Matrix *Y, tsne_var_t *var, int n, int n_dim, double momentum){
    // calculate low-dimensional affinities
    ...
    for (int i = 0; i < n; i+=4){
        ...
        values_left = _mm256_set_pd(v[12]+v[14], v[8]+v[10], v[4]+v[6], v[0]+v[2]); // correct
        values_right = _mm256_set_pd(v[13]+v[15], v[9]+v[11], v[5]+v[7], v[1]+v[3]); // correct
        ...
        mask_left = _mm256_castsi256_pd(_mm256_cmpeq_epi64(_mm256_castpd_si256(pos_grad_left), _mm256_castpd_si256(pos_delta_left)));
        mask_right = _mm256_castsi256_pd(_mm256_cmpeq_epi64(_mm256_castpd_si256(pos_grad_right), _mm256_castpd_si256(pos_delta_right)));
    ...
    const int mean_mask = 0b10001101; //setup such that we have [mean1 mean3 mean0 mean2]
    const int mean_mask2 = 0b11011000; //setup such that we have [mean0 mean2 mean1 mean3]
...
* grad_desc_vectorized
*/
void _grad_desc_vec2(Matrix *Y, tsne_var_t *var, int n, int n_dim, double momentum){
  // calculate low-dimensional affinities
...
*/
void _grad_desc_vec3(Matrix *Y, tsne_var_t *var, int n, int n_dim, double momentum){
  ...
  for (int i = 0; i < n; i+=4){
    ...
    for (int j = 0; j < n; j+=4){
      ...
      tmp3 = _mm256_mul_pd(_mm256_sub_pd(p3, q3), qnum3);
      tmp4 = _mm256_mul_pd(_mm256_sub_pd(p4, q4), qnum4);
      valueleft1 = _mm256_fmadd_pd(tmp1, _mm256_sub_pd(yfixleft1, yleft), valueleft1);
      valueleft2 = _mm256_fmadd_pd(tmp2, _mm256_sub_pd(yfixleft2, yleft), valueleft2);
      valueleft3 = _mm256_fmadd_pd(tmp3, _mm256_sub_pd(yfixleft3, yleft), valueleft3);
      valueleft4 = _mm256_fmadd_pd(tmp4, _mm256_sub_pd(yfixleft4, yleft), valueleft4);
      valueright1 = _mm256_fmadd_pd(tmp1, _mm256_sub_pd(yfixright1, yright), valueright1);
      valueright2 = _mm256_fmadd_pd(tmp2, _mm256_sub_pd(yfixright2, yright), valueright2);
      valueright3 = _mm256_fmadd_pd(tmp3, _mm256_sub_pd(yfixright3, yright), valueright3);
      valueright4 = _mm256_fmadd_pd(tmp4, _mm256_sub_pd(yfixright4, yright), valueright4);
      twoj += 8;
    }
